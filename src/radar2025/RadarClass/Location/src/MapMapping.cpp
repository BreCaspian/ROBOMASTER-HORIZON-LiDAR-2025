...
#else
// 实现一个通用的轨迹预测方法，与具体跟踪器无关
// 该方法可以与ByteTracker、DeepSort或其他跟踪器一起使用
void MapMapping::_GenericTracker_prediction(vector<bboxAndRect> &pred, vector<DetectBox> &sepboxs)
{
    map<int, int> new_trackid_map;
    
    for (const auto &it : pred)
    {
        if (it.rect.trackID >= 0) {  
            new_trackid_map[it.armor.cls] = it.rect.trackID;
        }
    }
    
    if (!this->_TrackID_cache.empty())
    {
        for (auto iter = this->_ids.begin(); iter != this->_ids.end(); ++iter)
        {
            int cls_id = iter->first;
            
            if (new_trackid_map.find(cls_id) == new_trackid_map.end() 
                && this->_TrackID_cache.find(cls_id) != this->_TrackID_cache.end())
            {
                int historical_track_id = this->_TrackID_cache[cls_id];
                
                for (auto &box : sepboxs)
                {
                    if (box.trackID == historical_track_id)
                    {
                        new_trackid_map[cls_id] = historical_track_id;
                        
                        float x = box.x1 + (box.x2 - box.x1) / 3.0f;
                        float y = box.y1 + ((box.y2 - box.y1) * 0.6f); 
                        float w = (box.x2 - box.x1) / 3.0f;
                        float h = (box.y2 - box.y1) / 5.0f;
                        bboxAndRect restored_box{
                            ArmorBoundingBox{true, x, y, w, h, (float)cls_id},
                            box
                        };

                        restored_box.rect.trackID = historical_track_id;

                        pred.push_back(restored_box);
                        
                        logger->debug("Restored tracked target: cls={}, trackID={}", cls_id, historical_track_id);
                        break;
                    }
                }
            }
    ...
    this->_TrackID_cache = new_trackid_map;
}
#endif
...
void MapMapping::push_T(Mat &rvec_input, Mat &tvec_input)
{
    ...
    this->logger->info("Note: tvec above represents world origin in camera frame, not camera position in world");
    
    Mat rvec_Matrix;
    Rodrigues(this->rvec, rvec_Matrix);
    
    stringstream ss_rot_matrix;
    ss_rot_matrix << "Rotation Matrix= " << endl
                 << " " << rvec_Matrix << endl
                 << endl;
    this->logger->info(ss_rot_matrix.str());
    ...
    stringstream ss_transform;
    ss_transform << "Transform Matrix (World->Camera)= " << endl
                << " " << T_Matrix << endl
                << endl;
    this->logger->info(ss_transform.str());
    
    cv2eigen(T_Matrix, this->T);
    
    this->_T << this->T.inverse();

    stringstream ss_inverse;
    ss_inverse << "Inverse Transform Matrix (Camera->World)= " << endl;
    for (int i = 0; i < 4; i++) {
        ss_inverse << " [";
        for (int j = 0; j < 4; j++) {
            ss_inverse << this->_T(i, j);
            if (j < 3) ss_inverse << ", ";
        }
        ss_inverse << "]" << endl;
    }
    ss_inverse << endl;
    this->logger->info(ss_inverse.str());
    
    Matrix<float, 4, 1> m1;
    m1 << 0.f, 0.f, 0.f, 1.f;
    this->cameraPostion << (this->_T * m1).topRows(3);
    
    stringstream ss_camera_pos;
    ss_camera_pos << "Camera Position in World= " << endl
                << " [" << this->cameraPostion(0,0) << ", "
                << this->cameraPostion(1,0) << ", "
                << this->cameraPostion(2,0) << "]" << endl
                << "This is the true position vector of camera relative to world origin" << endl;
    this->logger->info(ss_camera_pos.str());
    
    stringstream ss_intrinsics;
    ss_intrinsics << "--- System Parameters Summary ---" << endl
                 << "Camera ID: " << (this->_pass_flag ? "Calibrated" : "Not Calibrated") << endl
                 << "Transform Path: World -> Camera -> Image" << endl
                 << "Transform Used: ProjectPoints = K * [R|t] * WorldPoints" << endl;
    this->logger->info(ss_intrinsics.str());
