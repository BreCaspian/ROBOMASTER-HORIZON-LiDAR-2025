...
private:
    ...
    // 新增：通用跟踪器ID缓存，用于在不同类型的跟踪器间保持一致性
    map<int, int> _TrackID_cache; // 类别ID到跟踪ID的映射
...
#else
    // 新增：通用轨迹预测方法，可与任何跟踪器配合使用
    void _GenericTracker_prediction(vector<bboxAndRect> &pred, vector<DetectBox> &sepboxs);
#endif
